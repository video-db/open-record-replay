...
SCENE_INDEX_PROMPT = (
    "Multiple frames from this time interval are shown. Produce a compact, "
    "replay-focused scene summary for a skill compiler. Describe each distinct "
    "UI state in the interval, but avoid full screen dumps. Keep the total "
    "response under 900 characters unless a form, dropdown, file selection "
    "dialog, progress state, or confirmation screen needs extra detail.\n\n"
    "For each distinct state, include only replay-critical information:\n"
    "1. The visible app/site/page/dialog and the active task state.\n"
    "2. The element being interacted with or likely interaction target: field, "
    "button, menu item, dropdown, radio button, checkbox, tab, or file-picker "
    "control.\n"
    "3. Field contents, typed text, selected options, filenames, paths, table-row "
    "values, or dropdown options only when they affect replay.\n"
    "4. Visible status, progress, error, success, confirmation, loading, or final "
    "state text.\n"
    "5. Important state changes across frames, such as a modal opening, a page "
    "advancing, a value appearing, or a selection becoming checked.\n\n"
